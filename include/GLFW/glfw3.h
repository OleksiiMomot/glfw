 ...
 *
 *  This is the reference documentation for initialization and version related
 *  functions.  For more information, see the @ref intro.
 */
 ...
 *
 *  This occurs if a GLFW function was called that operates on the current
 *  context but no context is current.  One such function is @ref
 *  glfwSwapInterval.
 */
 ...
 *  [error](@ref intro_error) occurred.
 *
 ...
 *
 *  @return The primary monitor, or `NULL` if an [error](@ref intro_error)
 *  occurred.
 *
 *  @par Thread Safety
 *  This function may only be called from the main thread.
 ...
 *  key according to the current keyboard layout.  The returned string is
 ...
 *  allows you to pass unmodified the key token and scancode provided with key
 *  events.
 *
 *  @param[in] key The keyboard key to query, or `GLFW_KEY_UNKNOWN`.
 *  @param[in] scancode The scancode of the key to query, or zero.
 *  @return The UTF-8 encoded name of the key, or `NULL` if an
 *  [error](@ref intro_error) occurred.
 *
 *  @par Thread Safety
 *  This function may only be called from the main thread.
 ...
 *  @par Pointer Lifetime
 *  The returned string is allocated and freed by GLFW.  You should not free it
 *  yourself.  It is valid until this function is called again.
 *
 ...
 *  if an [error](@ref intro_error) occurred.
 *
 *  @par Pointer Lifetime
 *  The returned string is allocated and freed by GLFW.  You should not free it
 ...
 *  unavailable or an [error](@ref intro_error) occurred.
 *
